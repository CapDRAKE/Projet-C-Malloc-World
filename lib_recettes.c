...
//crafting est la seule fonction ayant une utilisation extC)rieure au script.
...
// Craft un item (en supprimant les ressources necessaires) et le place dans l'inventaire.
//renvoi 1 en cas de reussite, 0 s'il n'y a plus de place dans l'inventaire et -1 s'il n'y a pas assez de ressources. 
int
crafting(item** inventory, int code)
{
    int res = -1;
    ...
    {
        decrementDurInTab(inventory,
            findPosInInventory(inventory, recet->items[0]),
            recet->quantite[0]);
        decrementDurInTab(inventory,
            findPosInInventory(inventory, recet->items[1]),
            recet->quantite[1]);
